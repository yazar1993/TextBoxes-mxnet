    ...
    def _generate_anchors(self, sizes, ratios, step, alloc_size, offsets):
        ...
        for i in range(alloc_size[0]):
            for j in range(alloc_size[1]):
                ...
                anchors.append([cx,cy_1,sizes[0],sizes[0]])
                anchors.append([cx,cy_1,sizes[1],sizes[1]])
                # size = sizes[0], ratio = ...
                for r in ratios[1:]:
                    sr = np.sqrt(r)
                    w = sizes[0] * sr
